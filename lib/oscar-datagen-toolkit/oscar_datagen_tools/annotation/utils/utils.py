...
from oscar_datagen_tools.common_utils import get_sensor_type, verify_image_filename
...
image_path_pattern = re.compile(
    r"[A-Za-z0-9_]+/route[0-9]+_[0-9]*\.[0-9]+z_[0-9]*\.[0-9]+deg\.[0-9]+\.png",
    re.IGNORECASE,
)  # path patter similar to: rgb/route244542543218049066126211912380029915139_31.42z_281.95deg.00000004.png
...
def verify_dataset_path(path: Path, exclude_dirs: List[Path]) -> Dict[str, Any]:
    ...
    # Get the paths of those images that located 1 level deeper.
    # The expected path structure is: /<SENSOR>/<IMAGE>.png
    path_pattern = path / "*/*.png"
    image_paths = glob.glob(str(path_pattern))

    dataset_content = {SENSORS_KEY: defaultdict(list)}
    for image_path in image_paths:
        # get the 2 latest levels of the path
        relative_path = get_relative_path(Path(image_path), parent_level=2)
        # verify the format
        if not image_path_pattern.match(str(relative_path)):
            continue

        # the parent is expected to be the sensor
        sensor = relative_path.parent.name
        dataset_content[SENSORS_KEY][sensor].append(Path(image_path))
